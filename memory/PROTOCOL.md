...
session-start read. `(ADR-NNNN)` tags on continuity invariants are pointers only. If the
ledger exists, propose a newer ADR for a durable architecture decision or when
superseding/invalidating a tagged fact; mark the old ADR `Superseded`/`Deprecated`, never
delete it, keep `formalizes:` ↔ `(ADR-NNNN)` aligned, and wait for human approval
(`DECAY.md` §9, §12).
