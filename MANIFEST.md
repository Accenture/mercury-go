...
| 4.41.0 | 4.40.1 -> 4.41.0 | Protocol text changed (track referenced, created, reactivated, or closed ids; declare each closed thread): re-copy a target memory/PROTOCOL.md still byte-identical to the 4.40.1 template; arbitrate a customized one per ENABLE.md 5i. If memory-lint reports [overdue] on a thread closed within the window, declare the id retroactively in the next log instead of sweeping it. |
| 4.41.2 | 4.41.1 -> 4.41.2 | Protocol text changed (ADR paragraph: an entry is written only when a decision is accepted; proposals live in the repo's register, e.g. docs/arch-decisions/RFC.md with RFC-NNNN ids, never as a Proposed ADR): re-copy a target memory/PROTOCOL.md still byte-identical to the 4.41.1 template; arbitrate a customized one per ENABLE.md 5i. Optional by hand: change the header example id in memory/continuity.md to <kebab-id> (the parser fix covers it either way); if docs/arch-decisions/ADR.md holds Proposed entries, the human resolves them once. |
