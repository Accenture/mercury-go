...
> The tool kept its name and its version at graduation (v4.39.2 at the time) — graduation recognized
> the tool's state, it did not change it; enabled repositories needed no action for it.
...
| Version | Capability |
|---|---|
| 4.42.0 | **An opt-in seed for the governance pair — the `optional` policy:** maintainer proposal (RFC-0005) after two field repos adopted an ADR ledger by hand. A seventh MANIFEST policy, `optional`: installed only on an explicit `--adopt <target>`, never touched when present, listed for reference when absent — never pending, never re-asked on upgrade (plain `seed-copy` would have installed governance ceremony everywhere and re-offered a deliberate deletion). Skeleton templates for `docs/arch-decisions/ADR.md` + `RFC.md`; `ENABLE.md` Step 10 offers the pair once. No protocol change |
| 4.41.2 | **A phantom fact in every enabled repo, and an ADR ledger that records decisions only:** two mercury-composable reports. The seeded `continuity.md` header's example footer (inside an inline code span) parsed as a live `kebab-id` fact in every enabled repo — active, unpinned, counted toward `[continuity-bloat]`, raising nothing itself; in the field it fired `36 > 35` at a true count of 35 and summoned a review with nothing to archive. A backtick-wrapped footer is now documentation to `memory-lint`, the `[undeclared-reference]` mapper and `archive-fact`; the seeded header reads `id: <kebab-id>`. And the protocol no longer asks for a `Proposed` ADR: an entry is written only on acceptance, proposals live in the repo's register (e.g. `docs/arch-decisions/RFC.md`, `RFC-NNNN`). Protocol text changed → semantic row |
| 4.41.1 | **Thread ids name the thing, never their kind:** field note (mercury-composable, on v4.41.0, cosmetic): `memory/open-threads/thread-<id>.md` plus an id beginning `ot-`/`thread-` stutters (`thread-thread-….md` reads as a typo and is not) — near-universal across the family repos, and mostly the tool's doing (the mandated `ot-close-stalled-threads-<date>` gate id, the dogfood `ot-` habit, `thread-` ids in the example). Maintainer ruling, now the tool's rule: an id names the thing, never its kind, and an existing id is never renamed (immutable logs are `refresh-metadata`'s only input). Guidance in `DECAY.md` §1, the schema, `REVIEW.md` step 8 (new gates `close-stalled-threads-<date>`), `ENABLE.md`; the `thread-` filename prefix stays, its removal backlogged. No protocol change |
| 4.41.0 | **`[undeclared-reference]` — a fact edited without being declared:** field report (mercury-composable, on v4.40.1): at their first closure gate the maintainer closed two Blueprint gaps, the closing session rewrote both records and declared neither, and `refresh-metadata` read the human's decision as non-use — footers and reference log agreed, so only the diff could see it. New memory-lint check 16 over the staged index (pre-commit fragment) or a commit range (all three CI floors): a fact whose body changes must be declared in a session log staged with it; footer-only lines, condensed closed records, verbatim moves and deletions never count; silent when no log is staged; advisory. Measured over 60 memory commits in three repos, the premise held in 93–98% of commits and every repo carried undeclared edits. Closures are now declared by rule; `REVIEW.md`/`DECAY.md` corrected (the v4.40.0 wording named re-affirmation alone). Protocol text changed → semantic row |
| 4.40.1 | **Secret guard — punctuation-tolerant exemptions, and the waiver file as a last resort:** field note (mercury-composable, on v4.40.0): a comment sentence mentioning a setting — `credentials.source=OAUTHBEARER,` — flagged `[secret-material]` while the bare setting was exempt, because trailing sentence punctuation rides into the captured value; the same shape defeated every placeholder exemption except the tool's own knob. Fix: retry the exemptions once with trailing `).,` stripped, after the as-is pass (real values still flag; `$(…)` keeps matching). Maintainer decision on the team's suggestion: `.agent/secret-scan-ignore` is a **last-resort escape hatch** — no longer seeded or recommended; restructure fixtures to placeholders, since even dummy test values are false positives in field security scanners. Mechanism unchanged (a committed file is still honored; the hook states the implication). Protocol text changed → semantic row |
| 4.40.0 | **Stalled open threads — human closure gate:** field report (mercury-composable, on v4.39.2): an unchecked Open Thread is exempt from decay by design, and that exemption also removed the only automated freshness signal — a pinned thread rotted 184 sessions with four of its five "still open" items long shipped, and no lint check, review step, or metadata field could surface it; the tool's own repo had two such threads. New `thread_stale_window` knob (default 40) + `memory-lint` `[thread-stale]` advisory for detection, and `REVIEW.md` step 8: **one human closure gate** per review listing every stalled thread — the owner closes it (undelivered items recorded as *deliberately dropped*) or re-affirms it (a `## Memory References` entry, the only reset); the tool never closes a thread. Plus the one-lifecycle-per-record rule (a sub-list or status scalar cannot carry an independent commitment). Targets converge by re-copy; the knob is optional |
| 4.39.2 | **CI secret-scan waiver — silent abort fixed:** field report (mercury-composable PR #318, the first PR to exercise a `.agent/secret-scan-ignore` waiver there): when the LAST changed credential-class config file was waived, the exemption filter's trailing `[ "$keep" = "1" ] && printf ...` made the false test the loop's — and the command substitution's assignment's — exit status, so the runner's `set -e` killed the step with **zero output**: a red `memory` check that fires preferentially on a team's first legitimate waiver use and reads as CI flakiness. One-line `if ... fi` fix in all three forge floors (GitHub/GitLab/AzDO); class sweep clean (the pre-commit guard already used `if/else`); `tests/test_ci_secret_scan_waiver.sh` extracts each floor's loop verbatim and pins the motivating fixture + three neighbors under `-e`, red-verified. Targets converge by re-copy of the CI floor |
| 4.39.1 | **Same-thread merge claim — honest boundary:** an independent CoPilot assessment of v4.39.0 flagged that "editing the same thread conflicts per-file" overstates git: measured, conflicts arise only on adjacent/overlapping hunks, while separated edits merge cleanly with **both sides kept** (nothing lost; consistency is the write-time contradiction check's job, and the pre-4.39.0 layout merged identically). Docs state the measured boundary; the merge-contract test pins it with two new cases (separated → clean keep-both, adjacent → conflict; 4 → 6 cases). No behavior change |
...
agent-memory/
  ...
  templates/                         ← installed into target repos
    ...
    docs/arch-decisions/ADR.md, RFC.md  ← the optional governance pair (skeletons; adopted on request, never imposed — v4.42.0)
