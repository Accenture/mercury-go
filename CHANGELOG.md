...
- **Dogfooded the rule before release:** this repo opened `docs/arch-decisions/RFC.md` with five real
  proposals — protocol-text propagation, `seed-copy` "deliberately absent", the `thread-` filename prefix
  (parked), formalizing the ledger rule itself (RFC-0004, **promoted to ADR-0008** at the maintainer's gate
  the same day — the first proposal to travel the path it describes), and an opt-in seed for the governance
  pair (RFC-0005, maintainer go for v4.42.0). Each thread-backed proposal points at its thread and the
  thread points back.
